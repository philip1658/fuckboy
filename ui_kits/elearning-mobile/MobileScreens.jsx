...
function MKicker({ children, tone }) {
  return <div style={{ fontFamily: 'var(--font-condensed)', fontSize: '12px', fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: tone || 'var(--bordeaux-400)' }}>{children}</div>;
}
...
function MCover({ go }) {
  ...
  return (
    <div style={{ padding: '24px 20px 32px' }}>
      <MKicker>Über 200 Frauen · Anonym</MKicker>
      <div style={{ border: '2px solid var(--paper-100)', padding: '22px 16px 14px', marginTop: '16px' }}>
        <div style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: '36px', lineHeight: 0.88, letterSpacing: '-0.035em', color: 'var(--paper-100)' }}>FICK-TRAINER</div>
        <div style={{ borderTop: '1px solid var(--rule-strong)', marginTop: '14px', paddingTop: '9px', fontFamily: 'var(--font-condensed)', fontSize: '11px', fontWeight: 600, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'var(--paper-300)' }}>fick-trainer.de</div>
      </div>
      <p style={{ fontFamily: 'var(--font-sans)', fontSize: '19px', lineHeight: 1.5, color: 'var(--text-primary)', marginTop: '22px' }}>
        Ich bin nicht besonders gut aussehend. Ich habe trotzdem mit über 200 Frauen geschlafen, fast alle über Dating-Apps. Nicht wegen meinem Gesicht — wegen Psychologie: Erwartungssteuerung, Gesprächsführung, Timing. Dieselben Mechanismen, die ich beruflich im Verkauf benutze.
      </p>
      <p style={{ fontFamily: 'var(--font-sans)', fontSize: '19px', lineHeight: 1.555, color: 'var(--text-muted)', marginTop: '14px', borderLeft: '2px solid var(--bordeaux-700)', paddingLeft: '14px' }}>
        Ich schreibe anonym. Nicht aus Bescheidenheit — ich bin noch nicht fertig und benutze das hier weiter, während du es liest. Und ja: Anonymität erzeugt Autorität, „noch nicht fertig" erzeugt Dringlichkeit. Beides Techniken aus Kapitel 05. Sie funktionieren auch bei dir.
      </p>
      ...
      <div style={{ fontFamily: 'var(--font-display)', fontSize: '30px', fontWeight: 700, lineHeight: 1.05, letterSpacing: '-0.015em', color: 'var(--paper-100)', marginTop: '28px' }}>Welche Pille nimmst du?</div>
  ...
  return (
    <div style={{ paddingBottom: '28px' }}>
      <div style={{ ...PAD, paddingTop: '18px' }}>
        ...
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '40px', fontWeight: 700, lineHeight: 0.98, letterSpacing: '-0.02em', color: 'var(--paper-100)', marginTop: '11px' }}>Mehr Dates. Mehr Sex. Und die Rechnung dazu.</h1>
        <p style={{ fontFamily: 'var(--font-sans)', fontSize: '19px', lineHeight: 1.555, color: 'var(--text-secondary)', marginTop: '12px' }}>
      ...
      <div style={{ ...PAD, display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '14px', marginTop: '22px', paddingTop: '18px', borderTop: '1px solid var(--rule-subtle)' }}>
        {[['12', 'Kapitel'], [String(done).padStart(2, '0'), 'Fertig'], ['08', 'Kompetenzen']].map(([v, l]) => (
          <div key={l}>
            <div style={{ fontFamily: 'var(--font-data)', fontSize: '32px', fontWeight: 700, lineHeight: 0.9, color: 'var(--paper-100)' }}>{v}</div>
            <div style={{ fontFamily: 'var(--font-condensed)', fontSize: '12px', fontWeight: 600, letterSpacing: '0.13em', textTransform: 'uppercase', color: 'var(--text-faint)', marginTop: '7px' }}>{l}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ ...PAD, marginTop: '30px' }}>
        <Rule tone="accent" label="Was diesen Ratgeber unterscheidet" />
        <div style={{ display: 'grid', gap: '12px', marginTop: '16px' }}>
          {(window.FBData.gamechangers || []).map(g => (
            <Card key={g.t} surface="raised" padding="sm">
              <span style={{ display: 'flex', color: 'var(--volt-500)' }}><Icon name={g.icon} size={22} /></span>
              <div style={{ fontFamily: 'var(--font-display)', fontSize: '24px', fontWeight: 700, lineHeight: 1.1, letterSpacing: '-0.015em', color: 'var(--paper-100)', marginTop: '12px' }}>{g.t}</div>
              <p style={{ fontFamily: 'var(--font-sans)', fontSize: '16px', lineHeight: 1.6, color: 'var(--text-secondary)', marginTop: '8px' }}>{g.s}</p>
            </Card>
          ))}
        </div>
      </div>
      ...
      <div style={{ ...PAD, marginTop: '30px' }}>
        ...
        <p style={{ fontFamily: 'var(--font-sans)', fontSize: '16px', lineHeight: 1.6, color: 'var(--text-secondary)', marginTop: '14px' }}>
          Kein Tippkatalog, sondern eine Kette. Jeder Schritt hat eine Aufgabe — und eine eigene Abbruchstelle.
        </p>
        <div style={{ display: 'grid', gap: '2px', marginTop: '16px', borderTop: '1px solid var(--rule-subtle)' }}>
          {(window.FBData.flow || []).map(f => (
            <div key={f.n} style={{ padding: '14px 0', borderBottom: '1px solid var(--rule-faint)' }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '12px', flexWrap: 'wrap' }}>
                <span style={{ fontFamily: 'var(--font-data)', fontSize: '18px', fontWeight: 700, color: 'var(--bordeaux-400)' }}>{f.n}</span>
                <span style={{ fontFamily: 'var(--font-sans)', fontSize: '18px', fontWeight: 700, color: 'var(--paper-100)' }}>{f.t}</span>
                <span style={{ fontFamily: 'var(--font-condensed)', fontSize: '12px', fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--text-faint)' }}>{f.k}</span>
              </div>
              <div style={{ fontFamily: 'var(--font-sans)', fontSize: '16px', lineHeight: 1.55, color: 'var(--text-secondary)', marginTop: '7px' }}>{f.s}</div>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div style={{ paddingBottom: '28px' }}>
      <div style={{ ...PAD, paddingTop: '18px' }}>
        ...
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '40px', fontWeight: 700, lineHeight: 0.98, letterSpacing: '-0.02em', color: 'var(--paper-100)', marginTop: '11px' }}>Vom ersten Match bis zum Heimweg.</h1>
          ...
          <div key={c.number}>
            {c.image ? (
              <button onClick={() => go('ch:' + c.number)} aria-label={'Kapitel ' + c.number} style={{
                display: 'block', width: '100%', padding: 0, cursor: 'pointer', overflow: 'hidden', aspectRatio: '16 / 7',
                background: 'var(--ink-800)', border: '1px solid var(--rule-faint)', borderBottom: 0
              }}>
                <img src={'../../' + c.image} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
              </button>
            ) : null}
            <ChapterCard number={c.number} title={c.title} duration={c.duration}
...
function MSection({ s }) {
  ...
  if (s.type === 'text') return (
    <section style={wrap}>
      <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '28px', fontWeight: 600, lineHeight: 1.1, letterSpacing: '-0.01em', color: 'var(--paper-100)' }}>{s.h}</h2>
      {s.p.map((t, i) => <p key={i} style={{ fontFamily: 'var(--font-sans)', fontSize: '18px', lineHeight: 1.6, color: 'var(--text-secondary)', marginTop: '12px' }}>{t}</p>)}
    </section>
  );
  if (s.type === 'principle') return (
    <section style={wrap}><Card surface="raised" padding="sm">
      <MKicker tone="var(--wire-500)">{s.label}</MKicker>
      <div style={{ fontFamily: 'var(--font-display)', fontSize: '25px', fontWeight: 700, lineHeight: 1.12, color: 'var(--paper-100)', marginTop: '10px' }}>{s.h}</div>
      <p style={{ fontFamily: 'var(--font-sans)', fontSize: '18px', lineHeight: 1.6, color: 'var(--text-secondary)', marginTop: '10px' }}>{s.p}</p>
    </Card></section>
  );
            ...
            return (
              <Card key={k} surface="raised" padding="sm">
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center', color: good ? 'var(--volt-500)' : 'var(--bordeaux-400)' }}>
                  ...
                  <span style={{ fontFamily: 'var(--font-condensed)', fontSize: '12px', fontWeight: 600, letterSpacing: '0.13em', textTransform: 'uppercase' }}>{v.label || v.title}</span>
                </div>
                {v.text ? <div style={{ fontFamily: 'var(--font-sans)', fontSize: '19px', lineHeight: 1.555, color: 'var(--text-primary)', marginTop: '9px' }}>{v.text}</div> : null}
                {v.items ? (
                  <ul style={{ listStyle: 'none', margin: '10px 0 0', padding: 0, display: 'grid', gap: '7px' }}>
                    {v.items.map(t => <li key={t} style={{ fontFamily: 'var(--font-sans)', fontSize: '16px', lineHeight: 1.55, color: 'var(--text-secondary)' }}>{t}</li>)}
                  </ul>
                ) : null}
              </Card>
            );
      ...
      <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'grid', gap: '10px' }}>
        {s.items.map(t => (
          <li key={t} style={{ display: 'flex', gap: '11px', alignItems: 'flex-start' }}>
            ...
            <span style={{ fontFamily: 'var(--font-sans)', fontSize: '19px', lineHeight: 1.555, color: 'var(--text-secondary)' }}>{t}</span>
          </li>
        ))}
      </ul>
    </section>
      ...
      <div style={{ display: 'grid', gap: '2px' }}>
        {s.items.map(it => (
          <div key={it.k} style={{ padding: '11px 0', borderBottom: '1px solid var(--rule-faint)' }}>
            <div style={{ fontFamily: 'var(--font-condensed)', fontSize: '13px', fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--wire-500)' }}>{it.k}</div>
            <div style={{ fontFamily: 'var(--font-sans)', fontSize: '16px', lineHeight: 1.555, color: 'var(--text-secondary)', marginTop: '5px' }}>{it.v}</div>
          </div>
        ))}
      </div>
    </section>
  ...
  if (s.type === 'phrases') return (
    <section style={wrap}>
      ...
      {s.note ? <p style={{ fontFamily: 'var(--font-sans)', fontSize: '16px', lineHeight: 1.555, color: 'var(--text-muted)', marginBottom: '14px' }}>{s.note}</p> : null}
      <div style={{ display: 'grid', gap: '8px' }}>
        {s.items.map(t => <div key={t} style={{ padding: '12px 14px', background: 'var(--bg-surface)', borderLeft: '2px solid ' + (s.tone === 'good' ? 'var(--volt-600)' : 'var(--wire-600)'), fontFamily: 'var(--font-sans)', fontSize: '19px', lineHeight: 1.55, color: 'var(--text-primary)' }}>{t}</div>)}
      </div>
    </section>
  );
  return null;
    ...
    <div style={{ paddingBottom: '28px' }}>
      <div style={{ ...PAD, paddingTop: '18px' }}>
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <span style={{ fontFamily: 'var(--font-condensed)', fontSize: '12px', fontWeight: 600, letterSpacing: '0.13em', textTransform: 'uppercase', color: 'var(--text-muted)' }}>{ch.duration}{ch.quiz ? ' · ' + ch.quiz.length + ' Fragen' : ''}</span>
        </div>
        {ch.image ? (
          <div style={{ marginTop: '16px', aspectRatio: '4 / 3', overflow: 'hidden', border: '1px solid var(--rule-subtle)', background: 'var(--ink-800)' }}>
            <img src={'../../' + ch.image} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
          </div>
        ) : null}
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '44px', fontWeight: 700, lineHeight: 0.96, letterSpacing: '-0.025em', color: 'var(--paper-100)', marginTop: '14px' }}>{ch.title}</h1>
        <div style={{ fontFamily: 'var(--font-condensed)', fontSize: '13px', fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--bordeaux-400)', marginTop: '10px' }}>{ch.subtitle}</div>
        <p style={{ fontFamily: 'var(--font-sans)', fontSize: '19px', lineHeight: 1.5, color: 'var(--text-primary)', marginTop: '16px' }}>{ch.lead}</p>
          ...
          <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'grid', gap: '8px' }}>
            {ch.goals.map(g => (
              <li key={g} style={{ display: 'flex', gap: '11px', alignItems: 'flex-start' }}>
                ...
                <span style={{ fontFamily: 'var(--font-sans)', fontSize: '16px', lineHeight: 1.55, color: 'var(--text-secondary)' }}>{g}</span>
              </li>
            ))}
          </ul>
        </div>
        ...
        {ch.story ? (
          <div style={{ borderLeft: '2px solid var(--bordeaux-700)', paddingLeft: '14px', marginTop: '24px' }}>
            ...
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: '16px', lineHeight: 1.7, color: 'var(--text-secondary)', marginTop: '9px' }}>{ch.story}</p>
          </div>
        ) : null}
        ...
        {sc ? (
          <section style={{ marginTop: '30px' }}>
            ...
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '30px', fontWeight: 700, lineHeight: 1.06, letterSpacing: '-0.015em', color: 'var(--paper-100)' }}>{sc.title}</h2>
            <Card surface="sunken" padding="sm" style={{ marginTop: '14px' }}>
              {sc.brief.map((b, i) => <p key={i} style={{ fontFamily: 'var(--font-mono)', fontSize: '16px', lineHeight: 1.7, color: i === sc.brief.length - 1 ? 'var(--text-primary)' : 'var(--text-secondary)', marginTop: i ? '9px' : 0 }}>{b}</p>)}
            </Card>
            <div style={{ display: 'grid', gap: '8px', marginTop: '14px' }}>
              {sc.signals.map((s, i) => (
                <div key={i} style={{ display: 'flex', gap: '11px', alignItems: 'flex-start', padding: '10px 12px', background: 'var(--bg-surface)', border: '1px solid var(--rule-faint)' }}>
                  ...
                  <span style={{ fontFamily: 'var(--font-sans)', fontSize: '19px', lineHeight: 1.55, color: 'var(--text-secondary)' }}>{s.text}</span>
                </div>
              ))}
            </div>
        ...
        {ch.interaction ? (
          <section style={{ marginTop: '30px' }}>
            ...
            {ch.interaction.prompt ? <p style={{ fontFamily: 'var(--font-sans)', fontSize: '16px', lineHeight: 1.555, color: 'var(--text-muted)', marginBottom: '14px' }}>{ch.interaction.prompt}</p> : null}
        ...
        {ch.challenge ? (
          <section style={{ marginTop: '30px' }}>
            ...
            <Card surface="paper" padding="sm">
              ...
              <p style={{ fontFamily: 'var(--font-mono)', fontSize: '17px', lineHeight: 1.65, color: 'var(--ink-700)', marginTop: '9px' }}>{ch.challenge.text}</p>
    ...
    <div style={{ paddingBottom: '28px' }}>
      <div style={{ ...PAD, paddingTop: '18px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: '16px' }}>
        <div>
          ...
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '36px', fontWeight: 700, lineHeight: 1, letterSpacing: '-0.02em', color: 'var(--paper-100)', marginTop: '10px' }}>Kompetenzprofil</h1>
        </div>
        <div style={{ textAlign: 'right', flex: 'none' }}>
          <div style={{ fontFamily: 'var(--font-data)', fontSize: '38px', fontWeight: 700, lineHeight: 0.9, color: 'var(--paper-100)' }}>{done}<span style={{ color: 'var(--text-faint)', fontSize: '20px' }}>/12</span></div>
        </div>
      </div>
  ...
  return (
    <div style={{ paddingBottom: '28px' }}>
      <div style={{ ...PAD, paddingTop: '18px' }}>
        ...
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '40px', fontWeight: 700, lineHeight: 0.98, letterSpacing: '-0.02em', color: 'var(--paper-100)', marginTop: '11px' }}>Schutz &amp; Recht</h1>
        ...
        {S.groups.map(g => (
          <section key={g.id} style={{ marginTop: '28px' }}>
            <div style={{ display: 'flex', gap: '11px', alignItems: 'center' }}>
              ...
              <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '28px', fontWeight: 600, color: 'var(--paper-100)' }}>{g.title}</h2>
            </div>
            <div style={{ display: 'grid', gap: '2px', marginTop: '14px', borderTop: '1px solid var(--rule-subtle)' }}>
              {g.items.map(it => (
                <div key={it.k} style={{ padding: '12px 0', borderBottom: '1px solid var(--rule-faint)' }}>
                  <div style={{ fontFamily: 'var(--font-condensed)', fontSize: '13px', fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: TONE[g.tone] }}>{it.k}</div>
                  <div style={{ fontFamily: 'var(--font-sans)', fontSize: '17px', lineHeight: 1.6, color: 'var(--text-secondary)', marginTop: '6px' }}>{it.v}</div>
                </div>
              ))}
            </div>
          </section>
...
function MSettings({ store, onReset }) {
  ...
  return (
    <div style={{ ...PAD, paddingTop: '18px', paddingBottom: '28px' }}>
      ...
      <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '34px', fontWeight: 700, lineHeight: 1.02, color: 'var(--paper-100)', marginTop: '10px' }}>Fortschritt &amp; Daten</h1>
      <Card surface="ink" padding="sm" style={{ marginTop: '18px' }}>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: '17px', lineHeight: 1.75, color: 'var(--text-secondary)' }}>
